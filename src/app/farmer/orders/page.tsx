        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 10 }}>
          <div>
            ...
            <div style={{ fontSize: 13, color: "#6b7c71" }}>
              {o.buyerPhone} · #{o._id.slice(-6).toUpperCase()}
            </div>
          </div>
          ...
          <div style={{ display: "flex", gap: 8 }}>
            {acts.map((a) => (
              <button
                key={a.to}
                disabled={busy === o._id}
                onClick={() => update(o._id, a.to)}
                style={a.to === "cancelled" ? ghostBtn : primaryBtn}
              >
                {busy === o._id ? "..." : a.label}
              </button>
            ))}
  ...
  return (
    <div>
      <div style={{ display: "flex", alignItems: "center", gap: 14, marginBottom: 6 }}>
        ...
        <Link href="/farmer/listings" style={{ marginLeft: "auto", color: "#1f6b4a", fontWeight: 600, fontSize: 14 }}>
          Manage listings →
        </Link>
      </div>
      <p style={{ color: "#6b7c71", marginTop: 0, marginBottom: 18 }}>
        Incoming orders from customers. Confirm them and move them through to delivery.
      </p>
      ...
      {loading ? (
        ...
        <>
          <h2 style={{ fontSize: 18, fontWeight: 700, marginTop: 6 }}>
            Active {active.length ? `(${active.length})` : ""}
          </h2>
          {active.length === 0 ? (
            <p style={{ color: "#6b7c71" }}>No active orders right now.</p>
          ) : (
            <div style={grid}>{active.map(card)}</div>
          )}
          {past.length > 0 && (
            <>
              <h2 style={{ fontSize: 18, fontWeight: 700, marginTop: 26 }}>
